...
# Use "Occupancy" and other features to predict CO2 concentration
# Prepare data for regression
# Use "Occupancy" and other features to predict CO2 concentration
feature_columns = ["Occupancy"]
target_column = "Zone Air CO2 Concentration"

assembler = VectorAssembler(inputCols=feature_columns, outputCol="features")
regression_data = assembler.transform(spark_df).select("features", col(target_column).alias("label"), "Occupancy")

# Perform Linear Regression
print("Performing Linear Regression to Predict CO2 Concentration...")
...
# Include the "Occupancy" column in the predictions DataFrame
predictions_with_occupancy = predictions.select("features", "label", "prediction", "Occupancy")

# Show predictions
predictions_with_occupancy.show()

# Group predictions by 'Occupancy' and calculate statistics for CO2 predictions
stats = predictions_with_occupancy.groupBy("Occupancy").agg(
...
# Show statistics for each Occupancy value
print("Statistics for CO2 Predictions Grouped by Occupancy:")
stats.show()

# Show distinct features for each label (Occupancy)
predictions_with_occupancy.select("features", "Occupancy").distinct().show()