...
def process_co2_entropy(csv_file):
    ...
    df['presence_modified'] = df['presence_modified'].astype(bool)
    
    # Group by presence
    entropy_results = {}
    for presence_value in [True, False]:
        group_data = df[df['presence_modified'] == presence_value]['co2'].tolist()
        if group_data:
...
def process_co2_entropy_by_day(csv_file,day):
    ...
    for time, group in df.resample(window_size, on='Datetime'):
        ...
        for presence_value in [True, False]:
            co2_values = group[group['presence_modified'] == presence_value]['co2'].tolist()
...
if __name__ == "__main__":
    csv_file = "65m3_export_false_negative.csv"  # Change to your actual file path
    ...
    timestamps, entropy_values_plot = process_co2_entropy_by_day(csv_file,"2024-02-03")
