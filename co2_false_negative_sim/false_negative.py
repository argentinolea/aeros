...
df = df.withColumn("prev_occupancy", lag("BinaryOccupancy").over(window_spec))

df = df.withColumn("event_start", when((col("prev_occupancy") == 1) & (col("BinaryOccupancy") == 0), lit(1)).otherwise(lit(0)))

event_starts = [row["Datetime"] for row in df.filter("event_start == 1").select("Datetime").collect()]

override_flag_udf = udf(override_flag, BooleanType())
df = df.withColumn("override_occupancy", override_flag_udf(col("Datetime")))

df = df.withColumn(
    "BinaryOccupancy_modified",
    when(col("override_occupancy"), lit(1)).otherwise(col("BinaryOccupancy"))
)

presence_data = df.filter("BinaryOccupancy == 1").select("Datetime", "#occupants").collect()

get_last_occupants_udf = udf(get_last_occupants, IntegerType())
df = df.withColumn(
    "occupants_modified",
    when(col("override_occupancy"), get_last_occupants_udf(col("Datetime"))).otherwise(col("#occupants"))
)



CO2_occupant_dir = "10022002_modified_output"
df.select(
    "Datetime",
...
shutil.move(os.path.join(CO2_occupant_dir, csv_file), "10022002_modified_output.csv")
